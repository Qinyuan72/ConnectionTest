...
class RollRateTestApp:
    ...
    def __init__(self, root: tk.Tk, runtime: RollRateInnerLoopRuntime) -> None:
        ...
        self.base_rpm_var = tk.StringVar(value=f"{snapshot.command.base_rpm:.1f}")
        self.p_cmd_var = tk.StringVar(value=f"{snapshot.command.p_cmd_rad_s:.3f}")
        self.q_cmd_var = tk.StringVar(value=f"{getattr(snapshot.command, 'q_cmd_rad_s', 0.0):.3f}")
        self.kp_p_var = tk.StringVar(value=f"{snapshot.command.kp_p:.3f}")
        self.kp_q_var = tk.StringVar(value=f"{getattr(snapshot.command, 'kp_q', 0.0):.3f}")
        self.output_limit_var = tk.StringVar(value=f"{snapshot.command.output_limit:.3f}")
        self.mixer_name_var = tk.StringVar(value=snapshot.mixer_name)
